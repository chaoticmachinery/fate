...
#=============================================================================================
# Test the file to determine type of the file
#=============================================================================================
sub testfile {
    my ($srchfile) = @_;
    
    my($type,$typeuni,$regtype) = "";
    
    if ($srchfile =~ /\.LOG/) {
       return("File: $srchfile Type: Reg Log file.");
    }
    
    if (-p $srchfile) {
       return("File: $srchfile Type: FIFO Pipe.");
    }
    
    
    eval {
        open(FILE, $srchfile) or die("Error reading file, stopped");
        read(FILE, $typeuni, 160);
        close(FILE);    
        $type=decode('UTF-8',$typeuni);
        $type =~ s/[^a-zA-Z0-9\.\\,]//g;
        $type = lc $type;
        ...
        if ($type =~ /\\sytem32\\config\\default/) {
        	$regtype = "default";
        }     
        if ($type =~ /\\system32\\config\\software/) {
        	$regtype = "software";
        }      
        if ($type =~ /\\system32\\config\\sam/) {
        	$regtype = "sam";
        }
        if ($type =~ /\\system32\\config\\security/) {
        	$regtype = "security";
        }        
        if ($type =~ /ntuser\.dat/) {
        	$regtype = "ntuser";
        }
        if ($type =~ /windows\\usrclass\.dat/) {
        	$regtype = "usrclass";
        }               
    };
    if (lc($srchfile) =~ /recentfilecache/) {
       $regtype = "recentfilecache";
    }
    return($regtype);
}
#=============================================================================================
...
#		}
        ...
        if ($symblinkfile == 1) { 
            ...
            if ($symlinkerror != 1) {
                ...
                print "$symlinkerror -- Cannot create symbolic links: $!";
                }
            }
		}
	}
...
# Regripper Process the files
#=============================================================================================
sub regripperprocessplugin {
    
  my ($file, $reglog, $regtype) = @_;

  $reglog .= ".regripper.log";
  print "\tLog file (regripper): $reglog\n"  if $verbose;
  my $error = `$regripper -r \"$file\" -p $regtype > $reglog 2> /dev/null`; 
  #print "ERROR REGRIPPER: $error = $regripper -r \"$file\" -p $regtype\n"; 
  return;
}
#=============================================================================================
...
#=============================================================================================
# amcacheparser Process the files
#=============================================================================================
sub amcacheparser {
  ...
  $amlog = $reglog;
  $amlog .= ".amcache.log";
  print "\tLog file (regscan): $amlog\n"  if $verbose;
  my $error = `$amcacheparser \"$file\" > $amlog 2> /dev/null`;
  print "amcacheparser: $amcacheparser  \"$file\" > $amlog 2> /dev/null\n" if $verbose;
  
  $amlog = $reglog;
  $amlog .= ".amcache_regripper.log";
  print "\tLog file (regscan): $amlog\n"  if $verbose;
  my $error = `$regripper -r \"$file\" -p $amcacheparserreg > $amlog 2> /dev/null`;
  print "amcacheparser: $amcacheparserreg  \"$file\" > $amlog 2> /dev/null\n" if $verbose;
  return;
}
#=============================================================================================

#=============================================================================================
# Recentfilecache Process the files
#=============================================================================================
sub recentfileparser {
  
  my ($file, $reglog) = @_;

  $reglog .= ".recentfile.log";
  print "\tLog file (regscan): $reglog\n"  if $verbose;
  my $error = `$recentfileparser \"$file\" > $reglog 2> /dev/null`;
  print "recentfilecachepaser: $recentfileparser  \"$file\" > $reglog 2> /dev/null\n" if $verbose;
  return;
}
#=============================================================================================
...
#    * ·        $File::Find::name contains $File::Find::dir/$_
     ...
     #$srchfilename = lc($_);
     #$newfilename = "\""+$_+"\"";
     $srchfilename = testfile($_);
     print "F: $srchfilename\n";

      switch ($srchfilename) {
          case /recentfilecache/ {
	      my $fileout = `$filebin \"$File::Find::name\"`;
	      chomp($fileout);
	      my $reglog = "";
	      my $hash = "";
	      if ($fileout =~ /\bregistry\b/i) { 
		  print "\n\nWorking on $File::Find::name...   RecentFileCache\n";
		  ...
		  if ($srchfilename =~ /log/) {
		     ...
		     if ($File::Find::dir =~ /snapshot/) {
		          ...
		          if ($threadapp eq "n") {
				recentfileparser($File::Find::name, $orglog, "recentfilecache");
				...
				$thread = threads->create(\&recentfileparser,$File::Find::name, $orglog, "recentfilecache");
...
#				$thread = threads->create(\&regrestoreptprocess,$File::Find::name, $orglog);
	  ...
	  #case /amcache/ {
	      ...
	      my $hash = "";
	      if ($fileout =~ /\bregistry\b/i) { 
		  print "\n\nWorking on $File::Find::name...   Amcache Hive\n";
		  $orglog = copyfile($File::Find::dir,$File::Find::name,$_,1);
		  ...
		  if ($srchfilename =~ /log/) {
		     ...
		     if ($File::Find::dir =~ /snapshot/) {
		          ...
		          if ($threadapp eq "n") {
				amcacheparser($File::Find::name, $orglog, "amcache");
				#regtimeprocess($File::Find::name, $orglog, "HKCU_USERNAME");
				#regdumpprocess($File::Find::name, $orglog);
				#regscanprocess($File::Find::name, $orglog);
				#shellbagsparser($File::Find::name, $orglog);
			      } else {
				$thread = threads->create(\&amcacheparser,$File::Find::name, $orglog, "amcache");
				#$thread = threads->create(\&regtimeprocess,$File::Find::name, $orglog, "HKCU_USERNAME");
				#$thread = threads->create(\&regdumpprocess,$File::Find::name, $orglog);
				#$thread = threads->create(\&regscanprocess,$File::Find::name, $orglog);
				#$thread = threads->create(\&shellbagsparser,$File::Find::name, $orglog);
...
if (defined $config) {
    ...
    $maxthread=$Config->{regripper}->{maxthread};
    $amcacheparser=$Config->{regripper}->{amcacheparser};
    $amcacheparserreg=$Config->{regripper}->{amcacheparserreg};
    $recentfileparser==$Config->{regripper}->{recentfileparser};
    $copyfile==$Config->{regripper}->{copyfile};
    $symlinkfile==$Config->{regripper}->{symlinkfile};
  } else {
...
if ($threadapp eq "n") {
    ...
    find({ wanted => \&process, follow => 0,follow_skip ==1 }, $dir);
   } else {
    print "Working in threaded mode.\n";
    find({ wanted => \&threadprocess, follow => 0,follow_skip==1}, $dir);
