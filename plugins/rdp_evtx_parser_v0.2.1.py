        ...
        if eventid == 40:
           if channel == "Microsoft-Windows-TerminalServices-LocalSessionManager/Operational":
              try:
                 ...
                 rdpreason = str(subrec['Event']['UserData']['EventXML']['Reason'])
                 if rdpreason in rdpdisconnect:
                    lineout = lineout + "  Reason: " + rdpdisconnect[rdpreason] + "'|"  #7
                 else:
                    lineout = lineout + "  Reason: " + rdpreason + "'|"  #7
...
def main(argv):
   ...
   try:
      ...
      print ('rdp_evtx_parser.py -i <inputfile> -o <outputdir>')
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
         print ('rdp_evtx_parser.py -i <inputfile> -o <outputdir>')
