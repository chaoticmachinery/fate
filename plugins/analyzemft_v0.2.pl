...
sub mft_process{
    ...
    $mftfile = $dircwd . "/";
    $mftfile .= $file;
    $createdir = "\"" . $savedir."/".$File::Find::dir."\"";
    print "\tCreating directory: $createdir\n"; 
    
    my $status = system "mkdir -p $createdir";
    ...
    $cleanfile_BODYFILE=  "\"".addslash($File::Find::dir).$cleanfile."_BODYFILE\"";
    $cleanfile_CSVFILE =  "\"".addslash($File::Find::dir).$cleanfile."_CSVFILE.csv\"";
