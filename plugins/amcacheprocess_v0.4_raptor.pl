...
sub freq_analysis {
     ...
     my $nsrlapptypecmd = "mlr --fs '|' --csv cut -f amcache_SHA1,FullPath $mntdrive/*.csv | grep -v -f ".$nsrlsha1." |  mlr --fs '|' --csv cut -f FullPath | sort | uniq -c | sed -e 's/^[     ]*//' | sort -t' ' -k2";
...
if (defined $config) {
    ...
    $ir_file_distil_malwarelist=$Config->{MassTriage}->{ir_file_distil_malwarelist};
