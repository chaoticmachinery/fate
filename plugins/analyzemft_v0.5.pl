...
sub mft_process{
    ...
    my $tmpanalyzemftopts = $analyzemftopts;
    $tmpanalyzemftopts =~ s/SAVEDIR/\"$createdir/g;
...
B<analyzemft.pl> will parse all of the MFT files it finds in a given directory path. Ideally set the number of threads to 60 to 70% of the total number of cores that are available in the plugins.ini file. 
=cut
