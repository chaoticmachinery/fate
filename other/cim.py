...
class CIM(object):
    ...
    def guess_cim_type(path):
        tmpfile = os.path.join(path, '')
        tmpfile = tmpfile + "MAPPING1*.MAP*"
        tmpfileList = glob.glob(tmpfile)
        mappath = tmpfileList[0]
    ...
    def _data_file_path(self):
        tmpfile = os.path.join(self._directory, '')
        tmpfile = tmpfile + "OBJECTS*.DATA*"
        tmpfileList = glob.glob(tmpfile)
        tmpmappath = tmpfileList[0]
    ...
    def _current_mapping_file(self):
        ...
        for i in range(MAX_MAPPING_FILES):
            ...
            fnstart = "MAPPING{:d}".format(i + 1)
            fnend = "*.MAP*"
            tmpfile = os.path.join(self._directory, '')
            tmpfile = tmpfile + fnstart + fnend
            tmpfileList = glob.glob(tmpfile)
            tmpmappath = tmpfileList[0]
            fn = os.path.basename(tmpmappath)
            fp = tmpmappath
